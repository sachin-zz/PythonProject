...
def deposit():
    while True:
        amount = input("Enter the total deposit : ")
        if(amount.isdigit()):
            amount = int(amount)
            if(amount > 0):
                break
...
def get_number_of_lines():
    while True:
        lines = input("Enter the no.of lines from 1 to 3 : ")
        if(lines.isdigit()):
            lines = int(lines)
            if(1 <= lines <= 3):
                break
    ...
    while True:
        ...
        if(amount.isdigit()):
            ...
            if(MIN_BET <= amount <= MAX_BET):
                ...
                print(f"Enter amount between {MIN_BET} to {MAX_BET}")
        else:
            print("Please enter a number")
    return amount
...
def spin(balance):
    ...
    winnings, winning_lines = win(slots, lines, bet, symbol_value)
